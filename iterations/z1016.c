...
void reader(FILE* fp,FILE* fp1)
{
   ...
   free(buffer);
   output(fp1);
   /*printf("WORDDDD %s | loc: %d\n\n",mword[0].str,mword[0].loc[0]);
   printf("WORDDDD %s | loc: %d\n\n",mword[0].str,mword[0].loc[1]);
   printf("WORDDDD %s | loc: %d\n\n",mword[0].str,mword[0].loc[2]);
   printf("WORDDDD %s | loc: %d\n\n",mword[0].str,mword[0].loc[3]);
   printf("WORDDDD %s | loc: %d\n\n",mword[0].str,mword[0].loc[4]);
   printf("WORDDDD %s | loc: %d\n\n",mword[0].str,mword[0].loc[5]);
*/
}
...
void countSubstrings(char* buffer, int size)
{
   ...
   visited = (char*) calloc (size,sizeof(char));
   ...
   for(i=mnum;i<=(size/2)+1;i++)
   {
	...
	/* if(k>=size-i)
	  ...
	  *  w/sshich are to the right of sub1 that we have not seen before*/
	 if(!(alreadySeen(sub1,visited)))
	    for(j=0;j<=(size-(k+(i*2)));j++)
	    {
...
void output(FILE* fp1)
{
   ...
   if(fp1==NULL)
   {
	 ...
	 if(mword[i].count>0)
	 {
	    ...
	    mword[i].count++;
	    printf("%6d%6d %+*s    %-d",mword[i].length, mword[i].count,maxlength,mword[i].str,mword[i].loc[0]);
