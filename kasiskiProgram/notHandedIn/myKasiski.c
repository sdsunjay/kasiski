/******
 * MODIFIED AFTER HANDIN IN AN ATTEMPT TO MAKE FASTER.
 * DID NOT SUBMIT.
 */
...
{
   int i,j;
   /*support only for keys of length 100 or less*/ 
   int factor[100];

   for(i=0;i<mindex;i++)
   {
	 if(mword[i].count>1)
	    for(j=1;j<mword[i].count-1;j++)
	    {
	       if(gcd(mword[i].apart[j],mword[i].apart[j+1])>factor[j])
		     printf("here\n");
